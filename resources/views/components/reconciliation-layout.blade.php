    ...
    <body class="font-sans antialiased">
        ...
        <aside class="bob-sidebar">
            <div class="bob-sidebar-brand">
                ...
                <div>
                    <div class="text-sm font-bold text-white tracking-tight">BOB System</div>
                    <div class="text-[10px] font-semibold text-indigo-300/60 uppercase tracking-wider">Reconciliation</div>
                </div>
            </div>
            ...
            <nav class="flex-1 py-4 overflow-y-auto space-y-0.5">
                ...
                <a href="#" class="bob-sidebar-nav-item">
                    <svg class="w-[18px] h-[18px] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            ...
            <div class="shrink-0 px-4 py-4" style="border-top: 1px solid rgba(255,255,255,0.06);">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold shadow-lg" style="background: linear-gradient(135deg, #6366f1, #a855f7); color: white;">
                        {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="text-[13px] font-semibold truncate text-white">{{ Auth::user()->name }}</div>
                        <div class="text-[11px] truncate text-gray-500">{{ Auth::user()->roles->pluck('name')->first() ?? 'User' }}</div>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="p-2 rounded-lg text-gray-500 hover:text-white hover:bg-white/10 transition-all" title="Sign Out">
        ...
        <div class="bob-main">
            <header class="bob-topbar">
                <div>
                    ...
                    @endisset
                    @isset($pageSubtitle)
                        <p class="text-xs mt-0.5 text-slate-400 font-medium">{{ $pageSubtitle }}</p>
                    @endisset
                </div>
                <div class="flex items-center gap-3">
                    @isset($headerActions)
                        {{ $headerActions }}
                    @endisset
                    <div x-data="{ time: '' }" x-init="setInterval(() => { time = new Date().toLocaleTimeString('en-US', {hour: '2-digit', minute:'2-digit'}); }, 1000)" class="text-[11px] font-mono font-semibold px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-slate-400">
